...
try {
    ...
    $errors[] = 'Failed to ensure core tables: ' . htmlspecialchars($e->getMessage());
}

// Get departments and strands for dropdowns
$departments = [];
try {
    $departments = $conn->query('SELECT department_id AS id, name, code FROM departments WHERE is_active = 1 ORDER BY name')->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
...
if ($selectedDeptId > 0 && $departments) {
    foreach ($departments as $d) {
        if ((int)$d['id'] === $selectedDeptId) { $selectedDept = $d; break; }
    }
}
$selectedStrand = null;
if ($selectedStrandId > 0 && $strands) {
    foreach ($strands as $s) {
        if ((int)$s['id'] === $selectedStrandId) { $selectedStrand = $s; break; }
    }
}
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'add') {
            ...
            if ($name === '') {
                ...
                if ($postMode === 'strand') {
                    ...
                    if ($sid <= 0) { $errors[] = 'Please select a strand.'; }
                    else {
                        ...
                        $selectedStrandId = $sid; $mode = 'strand';
                    }
                } else {
                    $did = (int)($_POST['department_id'] ?? 0);
                    if ($did <= 0) { $errors[] = 'Please select a department.'; }
                    else {
                        ...
                        if (!$chk->fetch()) {
                            ...
                            $selectedDeptId = $did; $mode = 'department';
                        }
                    }
                }
            }
      ...
      <section class="bg-white rounded shadow p-4">
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full border">
            ...
            <tbody>
              <?php if (!$courses): ?>
                <tr><td colspan="4" class="p-3 text-center text-gray-500">No courses yet.</td></tr>
