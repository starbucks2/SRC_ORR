...
// Ensure departments table exists (idempotent) with department_id PK
try {
    ...
    $errors[] = 'Failed to ensure departments table: ' . htmlspecialchars($e->getMessage());
}
...
$departments = [];
try {
    $sql = "SELECT d.department_id AS id, d.name, d.code, d.is_active, COALESCE(c.cnt,0) AS course_count
            FROM departments d
            LEFT JOIN (
    ...
    <section class="bg-white rounded shadow p-4 mb-6">
      ...
      <form method="post" class="grid gap-3 md:grid-cols-3 items-end">
        ...
        <div>
          ...
          <input name="name" class="w-full border rounded p-2" placeholder="e.g., CCS" required>
        </div>
        <div>
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full border">
          ...
          <tbody id="dept-table-body">
            <?php if (!$departments): ?>
              <tr><td colspan="5" class="p-3 text-center text-gray-500">No departments yet.</td></tr>
