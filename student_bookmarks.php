...
try {
    ...
    $sql_all = "SELECT paper_id, bookmarked_at FROM bookmarks WHERE student_id = :student_id ORDER BY bookmarked_at DESC";
    ...
    foreach ($all_bookmarks as $bm) {
        ...
        // Try books table if not found in research_submission
        try {
            $sql_book = "SELECT book_id AS id, title, authors AS author, department, year, document FROM books WHERE book_id = ? LIMIT 1";
            $stmt_book = $conn->prepare($sql_book);
            $stmt_book->execute([$paper_id]);
...
<head>
    ...
    <style>
        ...
        @media (max-width: 1024px) {
            ...
            div.lg\:hidden.bg-blue-primary { display: none !important; }
...
?>
    ...
    (function(){
        ...
        if (fb && builtin) {
            fb.addEventListener('click', function(){ builtin.click(); });
        }
    ...
    async function doToggle(paperId, card) {
        try {
            ...
            if (result.success) {
                ...
                if (typeof Swal !== 'undefined') {
                    Swal.fire({ icon: 'error', title: 'Error', text: result.message || 'Failed to update bookmark.', confirmButtonText: 'OK' });
                } else {
