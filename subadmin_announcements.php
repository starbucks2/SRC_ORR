...
// Get the subadmin's department if they are a subadmin
$subadmin_department = '';
if (isset($_SESSION['subadmin_id'])) {
    try {
        $stmt = $conn->prepare("SELECT department FROM sub_admins WHERE id = ?");
...
try {
    // If user is a subadmin, only show announcements for their department
    if (isset($_SESSION['subadmin_id']) && !empty($subadmin_department)) {
        // Check if the department column exists
        $checkDept = $conn->prepare("SHOW COLUMNS FROM announcements LIKE 'department'");
        $checkDept->execute();
        if ($checkDept->rowCount() == 0) {
            // Create department column if it doesn't exist
            $conn->exec("ALTER TABLE announcements ADD COLUMN department VARCHAR(100) DEFAULT NULL");
        }
        
        $stmt = $conn->prepare("SELECT * FROM announcements WHERE department = ? ORDER BY created_at DESC");
        ...
        <form action="add_announcement.php" method="POST" class="mb-8">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div>
                    ...
                    <input type="datetime-local" id="open_at" name="open_at" required class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white dark:border-gray-600">
                </div>
                <div>
                    <label for="deadline" class="block text-gray-700 dark:text-gray-300 font-medium mb-2">Submission Deadline</label>
                    <input type="datetime-local" id="deadline" name="deadline" required class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white dark:border-gray-600">
                </div>
                <div>
                    <label for="department" class="block text-gray-700 dark:text-gray-300 font-medium mb-2">Target Department</label>
                    <?php if (isset($_SESSION['subadmin_id'])): ?>
                        <input type="text" value="<?= htmlspecialchars($subadmin_department) ?>" class="w-full px-4 py-2 border rounded-lg bg-gray-100 dark:bg-gray-600 dark:text-white" readonly>
                        <input type="hidden" name="department" value="<?= htmlspecialchars($subadmin_department) ?>">
                    <?php else: ?>
                        <select name="department" id="department" required class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white dark:border-gray-600">
        ...
        <div>
            ...
            <?php if ($announcements): ?>
                <?php foreach ($announcements as $announcement): ?>
                    <?php
                    $now = new DateTime();
                    $openAt = isset($announcement['open_at']) && !empty($announcement['open_at']) ? new DateTime($announcement['open_at']) : null;
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg mb-4 relative border-l-4 border-blue-500">
                        <div class="flex justify-between items-start mb-2">
                            <div>
                                ...
                                <?php if (!empty($announcement['department'])): ?>
                                    <span class="inline-block mt-1 px-2 py-1 bg-blue-100 text-blue-800 text-xs font-semibold rounded">
                                        <?= htmlspecialchars($announcement['department']) ?>
                                    </span>
                                <?php else: ?>
                                    <span class="inline-block mt-1 px-2 py-1 bg-gray-100 text-gray-800 text-xs font-semibold rounded">
                                        All Departments
                                    </span>
                                <?php endif; ?>
                            </div>
                        ...
                        <div class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                            <?php if (!empty($announcement['open_at'])): ?>
                                Opens: <?= date("F j, Y g:i A", strtotime($announcement['open_at'])) ?> ·
                            <?php endif; ?>
                            Deadline: <?= date("F j, Y g:i A", strtotime($announcement['deadline'])) ?>
                        </div>
                        
                        <div class="absolute top-4 right-4 flex space-x-2">
                            <a href="subadmin_edit_announcement.php?id=<?= $announcement['id'] ?>"
